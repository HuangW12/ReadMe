...
<div class="content">
  <div class="title">
	<h3 style="line-height: 1.3">求购列表</h3>
  </div>
  <c:forEach var="buy" items="${buyList }" varStatus="status" >
  <article class="excerpt excerpt-1" style="">             
		<header>
			<h2><a href="getBuyCommentlist?buyid=${buy.buyid}&uid=${buy.uid}&btitle=${buy.btitle}&bdate=${buy.bdate}&bcontent=${buy.bcontent}" title="${buy.btitle }&type=show" target="_blank" >${buy.btitle }</a>
			</h2>
		</header>
		<p class="meta">
			<time class="time"><i class="glyphicon glyphicon-time"></i> ${buy.bdate }</time>
		</p>
		<p class="note">${buy.bcontent}</p>
	</article>
	</c:forEach>
